...
const createSendToken = (user, statusCode, res) => {
    ...
    // remove user's password from output
...
exports.protect = catchAsync(async (req, res, next) => {
    ...
    // note: used promisify here b/c jwt.verify returns a promise - so in order to assign the promised value to a var - use await promisify to return value.
    ...
    if (!currentUser) {
        return next(new AppError('This token is not associated with a valid user', 401));
    }
...
// only for renderred pages - there will be no errors
